      ...
      <div class="row mx-auto d-flex flex-column gap-5 mt-4" style="width: 90%">
        ...
        >
          ...
          <div v-else class="w-100 mt-2">
            <lazy-benefits-drag-drop :isProfile="true" @selectedAvatarHandler="selectedAvatarHandler"/>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const selectedAvatarHandler = (file) => {
      ...
      ctx.emit("selectedAvatarHandler", selectedTab.value == "avatar" ? selectedAvatar.value : file);
    };
    return {
      closeModal,
      tabs,
